...
////////////////////////////////////////////////////////////////////////////////
void ExchangeOperator::rttd_apply_VXC_(double mix, Wavefunction& wf_ref,
  Wavefunction& dwf_ref, Wavefunction& wf, Wavefunction& dwf)
{
  // dwf += mix * ( |dwf_ref> <wf_ref|s_.wf> + |wf_ref><dwf_ref|s_.wf>
  // - |wf_ref><wf_ref|sigma_HF|wf_ref><wf_ref|s_.wf> )
  for ( int isp_loc = 0; isp_loc < wf.nsp_loc(); ++isp_loc )
  {
    ...
    {
      const Context &ctxt = wf.sd(isp_loc,ikp_loc)->c().context();
      if ( wf.sd(isp_loc,ikp_loc)->basis().real() )
      {
        DoubleMatrix c_proxy(wf.sd(isp_loc,ikp_loc)->c());
        DoubleMatrix dc_proxy(dwf.sd(isp_loc,ikp_loc)->c());
        DoubleMatrix cref_proxy(wf_ref.sd(isp_loc,ikp_loc)->c());
        DoubleMatrix dcref_proxy(dwf_ref.sd(isp_loc,ikp_loc)->c());
      ...
      {
        ComplexMatrix &c(wf.sd(isp_loc,ikp_loc)->c());
        ComplexMatrix &dc(dwf.sd(isp_loc,ikp_loc)->c());
        ComplexMatrix &cref(wf_ref.sd(isp_loc,ikp_loc)->c());
        ComplexMatrix &dcref(dwf_ref.sd(isp_loc,ikp_loc)->c());
...
////////////////////////////////////////////////////////////////////////////////
void ExchangeOperator::rt_apply_VXC_(int isp_loc, int ikp_loc, double mix, Wavefunction& wf_ref,
  Wavefunction& dwf_ref, Wavefunction& wf, Wavefunction& dwf)
{
  // dwf += mix * ( |dwf_ref> <wf_ref|s_.wf> + |wf_ref><dwf_ref|s_.wf>
  // - |wf_ref><wf_ref|sigma_HF|wf_ref><wf_ref|s_.wf> )
...
////////////////////////////////////////////////////////////////////////////////
void ExchangeOperator::rttd_apply_operator(Wavefunction& wf, Wavefunction& dwf)
{
  // apply sigmaHF to s_.wf and store result in dwf
  // use the reference function wf0_ and reference sigma(wf) dwf0_
  rttd_apply_VXC_(1.0, wf0_, dwf0_, wf, dwf);
}

////////////////////////////////////////////////////////////////////////////////
void ExchangeOperator::rt_apply_operator(int isp_loc, int ikp_loc, Wavefunction& wf, Wavefunction& dwf)
{
  // apply sigmaHF to s_.wf and store result in dwf
  // use the reference function wf0_ and reference sigma(wf) dwf0_
  rt_apply_VXC_(isp_loc, ikp_loc, 1.0, wf0_, dwf0_, wf, dwf);
}
