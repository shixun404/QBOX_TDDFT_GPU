 ...
 : s_(s), cd_(cd), wf_(s_.wf)
{
  ...
  for ( int isp_loc = 0; isp_loc < wf.nsp_loc(); ++isp_loc )
  {
    ...
    {
      nlp[isp_loc][ikp_loc] =
        new NonLocalPotential(s_.atoms, *wf.sd(isp_loc,ikp_loc), rtvp, vp);
    }
  }
 ...
 : s_(s), cd_(cd), wf_(wf)
{
  ...
  nlp.resize(wf_.nsp_loc());
  for ( int isp_loc = 0; isp_loc < wf_.nsp_loc(); ++isp_loc )
  {
    nlp[isp_loc].resize(wf_.nkp_loc());
    for ( int ikp_loc = 0; ikp_loc < wf_.nkp_loc(); ++ikp_loc )
    {
      nlp[isp_loc][ikp_loc] =
        new NonLocalPotential(s_.atoms, *wf_.sd(isp_loc,ikp_loc), rtvp, vp);
    }
  }
...
////////////////////////////////////////////////////////////////////////////////
void EnergyFunctional::update_nl_dk(D3vector deltakick_amp, RTPosition rtp)
{
  for ( int isp_loc = 0; isp_loc < wf_.nsp_loc(); ++isp_loc )
  {
    for ( int ikp_loc = 0; ikp_loc < wf_.nkp_loc(); ++ikp_loc )
    {
      nlp[isp_loc][ikp_loc]->update_twnl_dk(deltakick_amp, rtp, ikp_loc); 
    }
  }
}

////////////////////////////////////////////////////////////////////////////////
void EnergyFunctional::hpsi(int isp_loc, int ikp_loc, Wavefunction& psi, Wavefunction& hpsi )
{
  const bool use_confinement = s_.ctrl.ecuts > 0.0;

  hpsi.sd(isp_loc, ikp_loc)->c().clear();

  nlp[isp_loc][ikp_loc]->nlpsi(*psi.sd(isp_loc, ikp_loc), *hpsi.sd(isp_loc, ikp_loc));

  // Electric enthalpy
  // FOR GAMMA CALCULATION
  //if ( el_enth_ )
  //{
  //  el_enth_->enthalpy(dwf, compute_hpsi);
  //}

  tmap["hpsi"].start();
  const SlaterDet& sd = *(psi.sd(isp_loc,ikp_loc));
  SlaterDet& sdp = *(hpsi.sd(isp_loc,ikp_loc));
  const ComplexMatrix& c = sd.c();
  const Basis& wfbasis = wf_.sd(isp_loc, ikp_loc)->basis();
  ComplexMatrix& cp = hpsi.sd(isp_loc,ikp_loc)->c();
  const int mloc = cp.mloc();
  const double* kpg2 = wfbasis.kpg2_ptr();
  if ( rtvp ) kpg2 = vp->get_kpgpa2(wfbasis);
  ...
  if ( use_confinement )
  {
    for ( int n = 0; n < sd.nstloc(); n++ )
    {
      ...
      for ( int ig = 0; ig < ngwloc; ig++ )
        cp[ig+mloc*n] += 0.5 * ( kpg2[ig] + fstress[ig] ) * c[ig+mloc*n];
    }
  }
  else
  {
  ...
  // MODIFY WHEN IMPLEMENT HYBRID CALCULATION
  xco->rt_apply_self_energy(isp_loc, ikp_loc, psi, hpsi);
  tmap["hpsi"].stop();
}

////////////////////////////////////////////////////////////////////////////////
double EnergyFunctional::energy(Wavefunction& rwf, bool compute_hpsi, Wavefunction& dwf,
              ...
              bool compute_stress, valarray<double>& sigma)
{
  ...
  for ( int isp_loc = 0; isp_loc < rwf.nsp_loc(); ++isp_loc )
  {
    for ( int ikp_loc = 0; ikp_loc < rwf.nkp_loc(); ++ikp_loc )
    {
      const int ikpg = rwf.ikp_global(ikp_loc);
      const double weight = rwf.weight(ikpg);
      const SlaterDet& sd = *(rwf.sd(isp_loc,ikp_loc));
      const Basis& wfbasis = wf_.sd(isp_loc, ikp_loc)->basis();
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for ( int isp_loc = 0; isp_loc < rwf.nsp_loc(); ++isp_loc )
  {
    ...
    {
      const int ikpg = rwf.ikp_global(ikp_loc);
      enl_ += rwf.weight(ikpg) * nlp[isp_loc][ikp_loc]->energy(*rwf.sd(isp_loc, ikp_loc), 
		      compute_hpsi, *dwf.sd(isp_loc,ikp_loc), 
		      compute_forces, fion_enl_loc,
		      compute_stress, sigma_enl_loc);

      for ( int is = 0; is < nsp_; is++ )
        for ( int i = 0; i < 3*na_[is]; i++ )
          fion_enl[is][i] += rwf.weight(ikpg) * fion_enl_loc[is][i];

      if ( compute_stress )
        sigma_enl += rwf.weight(ikpg) * sigma_enl_loc;
    }
  }
  ...
  {
    const double wf_entropy = rwf.entropy();
...
////////////////////////////////////////////////////////////////////////////////
    ...
    // apply self-energy operator
    xco->rttd_apply_self_energy(rwf, dwf);
              ...
              bool compute_stress, valarray<double>& sigma)
{
  ...
  for ( int isp_loc = 0; isp_loc < wf_.nsp_loc(); ++isp_loc )
  {
    for ( int ikp_loc = 0; ikp_loc < wf_.nkp_loc(); ++ikp_loc )
    {
      ...
      const SlaterDet& sd = *(wf_.sd(isp_loc,ikp_loc));
...
////////////////////////////////////////////////////////////////////////////////
  ...
  for ( int isp_loc = 0; isp_loc < wf_.nsp_loc(); ++isp_loc )
  {
    ...
    {
      const int ikpg = wf_.ikp_global(ikp_loc);
      enl_ += wf_.weight(ikpg) * nlp[isp_loc][ikp_loc]->energy(compute_hpsi,
              *dwf.sd(isp_loc,ikp_loc), compute_forces, fion_enl_loc,
              compute_stress, sigma_enl_loc);

      for ( int is = 0; is < nsp_; is++ )
        for ( int i = 0; i < 3*na_[is]; i++ )
          fion_enl[is][i] += wf_.weight(ikpg) * fion_enl_loc[is][i];

      if ( compute_stress )
        sigma_enl += wf_.weight(ikpg) * sigma_enl_loc;
    }
  }
